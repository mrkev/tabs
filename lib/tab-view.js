...
module.exports = TabView = class TabView {
  ...
  handleEvents() {
    ...
    if (typeof this.pane.onItemDidTerminatePendingState === "function") {
      ...
      const onDidTerminatePendingStateDisposable = this.item.onDidTerminatePendingState(
        () => this.clearPending()
      );
      if (Disposable.isDisposable(onDidTerminatePendingStateDisposable)) {
        ...
        console.warn(
          "::onDidTerminatePendingState does not return a valid Disposable!",
          this.item
        );
      }
    }
    ...
    if (typeof this.item.onDidChangeTitle === "function") {
      const onDidChangeTitleDisposable = this.item.onDidChangeTitle(
        titleChangedHandler
      );
    ...
    if (typeof this.item.onDidChangePath === "function") {
      const onDidChangePathDisposable = this.item.onDidChangePath(
        pathChangedHandler
      );
