...
module.exports = {
  activate(state) {
    ...
    this.updateTraversalKeybinds = function() {
      ...
      let bindings = atom.keymaps.findKeyBindings({
        ...
        keystrokes: 'ctrl-tab'});
      if ((bindings.length > 1) && (bindings[0].source !== keyBindSource)) { return; }
    ...
    this.subscriptions.add(typeof atom.keymaps.onDidLoadUserKeymap === 'function' ? atom.keymaps.onDidLoadUserKeymap(() => this.updateTraversalKeybinds()) : undefined);
    ...
    const paneContainers = {
      center: (left = (typeof atom.workspace.getCenter === 'function' ? atom.workspace.getCenter() : undefined)) != null ? left : atom.workspace,
      left: (typeof atom.workspace.getLeftDock === 'function' ? atom.workspace.getLeftDock() : undefined),
      right: (typeof atom.workspace.getRightDock === 'function' ? atom.workspace.getRightDock() : undefined),
      bottom: (typeof atom.workspace.getBottomDock === 'function' ? atom.workspace.getBottomDock() : undefined)
    };

    return Object.keys(paneContainers).forEach(location => {
      const container = paneContainers[location];
      if (!container) { return; }
      return this.subscriptions.add(container.observePanes(pane => {
        ...
        return pane.onDidDestroy(() => _.remove(this.mruListViews, mruListView));
      })
      );
    });
  },
  ...
  deactivate() {
    ...
    for (let tabBarView of Array.from(this.tabBarViews)) { tabBarView.destroy(); }
    for (let mruListView of Array.from(this.mruListViews)) { mruListView.destroy(); }
  },
