...
module.exports = TabBarView = class TabBarView {
  constructor(pane, location) {
    ...
    this.subscriptions.add(
      atom.commands.add(this.pane.getElement(), {
        ...
        "tabs:close-tabs-to-right": () =>
          this.closeTabsToRight(this.getActiveTab()),
        "tabs:close-tabs-to-left": () =>
          this.closeTabsToLeft(this.getActiveTab()),
    ...
    this.subscriptions.add(
      atom.config.observe(
        "tabs.tabScrolling",
        this.updateTabScrolling.bind(this)
      )
    );
    this.subscriptions.add(
      atom.config.observe("tabs.tabScrollingThreshold", () =>
        this.updateTabScrollingThreshold()
      )
    );
    this.subscriptions.add(
      atom.config.observe("tabs.alwaysShowTabBar", () =>
  ...
  removeTabForItem(item) {
    ...
    for (tab of Array.from(this.getTabs())) {
      tab.updateTitle();
    }
    return this.updateTabBarVisibility();
  }
  ...
  openInNewWindow(tab) {
    ...
    const pathsToOpen = [atom.project.getPaths(), itemURI].reduce(
      (a, b) => a.concat(b),
      []
    );
    return atom.open({
  ...
  splitTab(fn) {
    ...
    if (
      (item =
        this.rightClickedTab != null ? this.rightClickedTab.item : undefined)
    ) {
      let copiedItem;
      if (
        (copiedItem = typeof item.copy === "function" ? item.copy() : undefined)
      ) {
  ...
  closeSavedTabs() {
    return (() => {
      ...
      for (let tab of Array.from(this.getTabs())) {
        if (
          !(typeof tab.item.isModified === "function"
            ? tab.item.isModified()
            : undefined)
        ) {
          result.push(this.closeTab(tab));
  ...
  getWindowId() {
    return this.windowId != null
      ? this.windowId
      : (this.windowId = atom.getCurrentWindow().id);
  }

  shouldAllowDrag() {
    return (
      this.paneContainer.getPanes().length > 1 ||
  ...
  onDragLeave(event) {
    ...
    if (!event.currentTarget.contains(event.relatedTarget)) {
      ...
      this.lastDropTargetIndex = null;
      return Array.from(this.getTabs()).map(
        tab => (tab.element.style.maxWidth = "")
      );
    }
  }
  ...
  onDrop(event) {
    ...
    const hasUnsavedChanges =
      event.dataTransfer.getData("has-unsaved-changes") === "true";
    ...
    if (fromWindowId === this.getWindowId()) {
      ...
      if (item != null) {
        return this.moveItemBetweenPanes(
          fromPane,
          fromIndex,
          toPane,
          toIndex,
  ...
  updateTabScrollingThreshold() {
    return (this.tabScrollingThreshold = atom.config.get(
      "tabs.tabScrollingThreshold"
    ));
  }
  ...
  removeDropTargetClasses() {
    ...
    for (var dropTarget of Array.from(
      workspaceElement.querySelectorAll(".tab-bar .is-drop-target")
    )) {
    ...
    return (() => {
      const result = [];
      for (dropTarget of Array.from(
        workspaceElement.querySelectorAll(".tab-bar .drop-target-is-after")
      )) {
...
var isAtomEvent = function(event) {
...
var itemIsAllowed = function(event, location) {
  for (let item of Array.from(event.dataTransfer.items)) {
    if (
      item.type === "allow-all-locations" ||
      item.type === `allowed-location-${location}`
    ) {
      return true;
    }
  }
