...
class IconServices {
  ...
  updateMRUIcon(view) {
    if (this.elementIcons) {
      ...
      this.elementIconDisposables.add(
        this.elementIcons(view.firstLineDiv, view.itemPath)
      );
    } else {
      let typeClasses = this.fileIcons.iconClassForPath(
        view.itemPath,
        "tabs-mru-switcher"
      );
      if (typeClasses) {
        if (!Array.isArray(typeClasses))
          typeClasses = typeClasses.split(/\s+/g);
  ...
  updateTabIcon(view) {
    ...
    if (view.iconName) {
      ...
      if (this.elementIcons) {
        ...
        view.iconElement = this.elementIcons(view.itemTitle, view.path, {
          isTabIcon: true
        });
